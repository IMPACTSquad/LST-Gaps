...
def dual_graph(image, mask, k=10):
    ...
    one_a = kneighbors_graph(
        image[..., 0].reshape(-1, image.shape[2]),
        k,
        include_self=False,
    )
    logging.info("First graph found")
    ...
    one_a = scipy.sparse.csr_matrix(
        csr_rows_set_nz_to_val(
            scipy.sparse.csr_matrix(one_a.T),
            np.argwhere(mask[..., 0, 0].flatten() == 0).flatten(),
        ).T
    )

    # graph using observed parts of second image
    two_a = kneighbors_graph(
        image[..., 1].reshape(-1, image.shape[2]),
        k,
        include_self=False,
    )
    logging.info("Second graph found")
    ...
    two_a = scipy.sparse.csr_matrix(
        csr_rows_set_nz_to_val(
            scipy.sparse.csr_matrix(two_a.T),
            np.argwhere(mask[..., 0, 1].flatten() == 0).flatten(),
        ).T
    )
