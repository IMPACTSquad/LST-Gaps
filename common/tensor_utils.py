...
if __name__ == '__main__':
    # G = np.array([[[1, 5, 9], [3, 7, 11]], [[2, 6, 10], [4, 8, 12]]])  # shape 2, 2, 3
    G = np.array([[[1, 3, 7], [5, 11, 2]], [[8, 4, 12], [6, 9, 10]]])  # shape 2, 2, 3
    print(f"Core shape: {G.shape}")
    A0 = np.array([[1, 2], [4, 2], [-1, 2], [0, 2], [10, 3]])
    A1 = np.array([[2, 3], [2, 1], [2, 1], [2.2, 2], [-1, 1]])
    A2 = np.array([[0, 0, 10], [0, 0, 3], [0, 0, -4], [0, 0, -2], [0, 0, 0]])

    after_one = mode_k_product(G, A0, 0)
    after_two = mode_k_product(after_one, A1, 1)
    after_three = mode_k_product(after_two, A2, 2)

    # G_0ij * A0_:0 + G_1ij * A0_:1  # the mode-0 result

    i = 1
    j = 2
    print((A1[i, :][None, :, None] * (A2[j, :][None, None, :] * G)).sum(axis=(1, 2)))
    print(A0.dot((A1[i, :][None, :, None] * (A2[j, :][None, None, :] * G)).sum(axis=(1, 2))))
    print(after_three[:, i, j])
